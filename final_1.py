...
class Linkedlist:
    ...
    def back(self, item, d):    # 뒤로 한 칸 이동
        ...
        temp.data = item
        if self.find(3-item) == None:   # 만약 말이 이동해서 다른 말을 잡았다면 연결리스트에 다른 말의 값을 갖는 노드가 없을 것
...
# 30정
# 헤드 노드가 없는 순환 이중 연결 리스트 / 16개의 노드로 구성
# 2명의 플레이어는 서로 대각선 위치에서 시작
# 상대의 노드에 도착하면 상대를 잡고 이김
# 6,6 이면 이동 방향 바꿈 - 이동 없이 방향만 바꾸고 상대에게 턴을 넘 - 기본은 시계방향 이동
# 5,5 이면 말의 위치를 서로 교환 - 위치 교환 후 상대에서 턴이 넘어감
# 1,1 이면 뒤로 한 칸 이동
# 그 외의 눈이 나오면 두 눈의 합 만큼 이동
